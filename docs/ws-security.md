...
1. **Always use HTTPS** - Even with WS-Security, transport-level encryption is recommended
2. **Use digest passwords** - Avoid plain text passwords when possible
3. **Use SHA-256 or stronger** - SHA-1 is considered weak for new implementations
4. **Protect private keys** - Store encrypted and with appropriate file permissions
5. **Use appropriate expiration times** - Balance security (shorter) with clock skew tolerance (longer)
6. **Verify responses** - Enable `verify_response` for high-security scenarios
7. **Sign WS-Addressing** - If using WS-Addressing, enable `sign_addressing` to prevent routing attacks

## Limitations

The current implementation does not support:

- XML Encryption (encrypting message content)
- SAML tokens
- Kerberos tokens
- Derived keys
