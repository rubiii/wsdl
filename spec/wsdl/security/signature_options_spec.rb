...
describe WSDL::Security::SignatureOptions do
  describe 'DEFAULTS' do
    it 'has sign_body defaulting to true' do
      expect(described_class::DEFAULTS[:sign_body]).to be true
    end
  ...
  describe '.from_hash' do
    context 'with empty hash' do
      ...
      it 'uses default values for all options' do
        expect(options.sign_body?).to be true
    ...
    context 'with partial hash' do
      subject(:options) { described_class.from_hash(sign_body: false, sign_addressing: true) }

      it 'uses provided values' do
        expect(options.sign_body?).to be false
  ...
  describe '#sign_body?' do
    it 'returns true when sign_body is true' do
      options = described_class.from_hash(sign_body: true)
      expect(options.sign_body?).to be true
    end

    it 'returns false when sign_body is false' do
      options = described_class.from_hash(sign_body: false)
      expect(options.sign_body?).to be false
    end

    it 'returns false when sign_body is nil' do
  ...
  describe '#==' do
    ...
    context 'when sign_body differs' do
      let(:options2) do
        described_class.from_hash(
          sign_body: false,
          sign_addressing: true,
    ...
    context 'when comparing with nil' do
      it 'returns false' do
        expect(options1.nil?).to be false
      end
    end
  end
  ...
  describe '#eql?' do
    it 'is aliased to ==' do
      options1 = described_class.from_hash(sign_body: true)
      options2 = described_class.from_hash(sign_body: true)
  ...
  describe '#hash' do
    it 'returns the same hash for equal options' do
      options1 = described_class.from_hash(sign_body: true, key_reference: :issuer_serial)
      options2 = described_class.from_hash(sign_body: true, key_reference: :issuer_serial)
    ...
    it 'returns different hashes for different options' do
      options1 = described_class.from_hash(sign_body: true)
      options2 = described_class.from_hash(sign_body: false)
