...
RSpec.describe 'DataExchange' do
  subject(:client) { WSDL::Client.new WSDL.parse(fixture('wsdl/data_exchange')) }
  ...
  it 'raises an error because RPC/encoded operations are not' do
    ...
    expect { client.operation(service, port, 'submit') }
      .to raise_error(WSDL::UnsupportedStyleError, %r{"submit" is an "rpc/encoded" style operation})
  end
end
